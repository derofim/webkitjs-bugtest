...
class Target:
  ...
  def Linkable(self):
    ...
    return self.type in ('static_library', 'shared_library', 'js_library')
...
class NinjaWriter:
  ...
  def WriteSourcesForArch(self, ninja_file, config_name, config, sources,
    ...
    if config.get('cflags_c',[]):
      cflags_c = config.get('cflags_c', [])

    if config.get('cflags_cc', []):
    ...
    cc_foo = config.get('cc', [])

    if cc_foo != '':
      ninja_file.variable('cc',cc_foo,0)

    cxx_foo = config.get('cxx', [])

    if cxx_foo != '':
      ninja_file.variable('cxx',cxx_foo,0)

    ld_foo = config.get('ld', [])

    if ld_foo != '':
      ninja_file.variable('ld',ld_foo,0)
      ninja_file.variable('ldxx',ld_foo,0)
  ...
  def WriteLinkForArch(self, ninja_file, spec, config_name, config,
    ...
    if command in ('solink', 'solink_module', 'solink_js'):
      ...
      elif not self.is_mac_bundle and command != 'solink_js':
        output = [output, output + '.TOC']
      elif command != 'solink_js':
  ...
  def ComputeOutputFileName(self, spec, type=None):
    ...
    DEFAULT_PREFIX = {
      ...
      'js_library': default_variables['JS_LIB_PREFIX'],
    ...
    DEFAULT_EXTENSION = {
        ...
        'js_library': default_variables['JS_LIB_SUFFIX'],
    ...
    if type in ('static_library', 'loadable_module', 'shared_library', 'js_library',
                        'executable'):
...
def CalculateVariables(default_variables, params):
  ...
  if flavor == 'mac':
    ...
    default_variables.setdefault('SHARED_LIB_SUFFIX', '.o')
